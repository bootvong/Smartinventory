...
Public Class tbcolor
    ...
    Public Function save(cate_id As Integer, name As String)
        cn.connect()
        Try
            cm = New SqlCommand("insert into tbcolor values('" & cate_id & "',N'" & name & "')", cn.conn)
    ...
    Public Function delete(cate_id As Integer)
        cn.connect()
        Try
            cm = New SqlCommand("delete from tbcolor where colorid='" & cate_id & "'", cn.conn)
    ...
    Public Function update(cate_id As Integer, name As String)
        cn.connect()
        Try
            cm = New SqlCommand("update tbcolor set colorname=N'" & name & "' where colorid='" & cate_id & "'", cn.conn)
    ...
    Public Function runid()
        ...
        Dim id As Integer
        Try
            cm = New SqlCommand("select top 1 colorid from tbcolor order by colorid desc", cn.conn)
            re = cm.ExecuteReader
            If re.HasRows Then
                While re.Read
    ...
    Public Function loadcolor(dgv As DataGridView)
        cn.connect()
        Try
            da = New SqlDataAdapter("select * from tbcolor", cn.conn)
            da.Fill(ds, "pt")
            ds.Tables.Clear()
            da.Fill(ds, "pt")
        ...
        Return True
    End Function
    Public Function comcolor(cb As ComboBox)
        cn.connect()
        ...
        Try
            ...
            With cb
                .DataSource = dt
                .DisplayMember = dt.Columns("colorname").ToString
                .ValueMember = dt.Columns("colorid").ToString
            End With
        Catch ex As Exception
            MessageBox.Show(ex.Message)
        End Try
